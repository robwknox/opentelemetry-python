...
CERT_FILE = "sender/fixtures/service.crt"
...
class TestGrpcSender(unittest.TestCase):
    def test_constructor_defaults(self):
        sender = GrpcSender(endpoint="test.endpoint.com", cert_file=CERT_FILE)
        self.assertEqual(sender._endpoint, "test.endpoint.com")
        self.assertEqual(sender._insecure, False)
        self.assertEqual(sender._cert_file, CERT_FILE)
    ...
    def test_constructor_default_requires_cert_file(self):
        with self.assertRaises(ValueError) as cm:
            sender = GrpcSender(endpoint="test.endpoint.com")
        self.assertEqual(
    ...
    def test_constructor_invalid_cert_file(self):
        with self.assertRaises(ValueError) as cm:
            sender = GrpcSender(endpoint="test.endpoint.com", cert_file="fake")
        self.assertEqual(
            cm.exception.args[0],
            "Unable to read credentials from file: 'fake'",
        )
    ...
    def test_send_success(self, stub_mock):
        sender = GrpcSender("test.endpoint.com", cert_file=CERT_FILE)
        export_request = ExportTraceServiceRequest()
