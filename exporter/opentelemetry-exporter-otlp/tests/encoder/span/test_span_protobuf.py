...
class TestSpanProtobufEncoder(unittest.TestCase):
    def test_encode(self):
        otel_spans, expected_encoding = self.get_exhaustive_test_spans()
        self.assertEqual(
            SpanProtobufEncoder().encode(otel_spans), expected_encoding
        )

    def test_serialize(self):
        otel_spans, expected_encoding = self.get_exhaustive_test_spans()
        self.assertEqual(
            SpanProtobufEncoder().serialize(otel_spans),
            expected_encoding.SerializeToString(),
        )

    def test_content_type(self):
        self.assertEqual(
            SpanProtobufEncoder.content_type(), "application/x-protobuf"
        )
    ...
    def get_exhaustive_otel_span_list() -> List[SDKSpan]:
        ...
        span2.resource = SDKResource(
            attributes={"key_resource": "some_resource"}
        )
        span2.end(end_time=end_times[1])

        span3 = SDKSpan(name="test-span-3", context=other_context, parent=None)
        span3.start(start_time=start_times[2])
        span3.set_attribute("key_string", "hello_world")
        span3.resource = SDKResource(
            attributes={"key_resource": "some_resource"}
        )
        span3.end(end_time=end_times[2])

        span4 = SDKSpan(name="test-span-4", context=other_context, parent=None)
        span4.start(start_time=start_times[3])
        span4.resource = SDKResource({})
        span4.end(end_time=end_times[3])
        span4.instrumentation_info = SDKInstrumentationInfo(
    ...
    def get_exhaustive_test_spans(
        ...
        span_kind = SPAN_KIND_MAP[SDKSpanKind.INTERNAL]
                        ...
                        PB2InstrumentationLibrarySpans(
                            ...
                            spans=[
                                PB2SPan(
                                    ...
                                    status=PB2Status(
                                        code=PB2Status.STATUS_CODE_UNKNOWN_ERROR,
